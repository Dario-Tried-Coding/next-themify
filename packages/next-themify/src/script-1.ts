...
import { CS_Sanitization, CS_Update, Custom_SE, HVs_Sanitization, HVs_Update, Script_Params, SM_Sanitization, SM_Update } from './types/script'
...
export function script({ config_SK, mode_SK, custom_SEK, config, constants: { STRATS, MODES, COLOR_SCHEMES } }: Script_Params) {
  ...
  // #region HELPERS --------------------------------------------------------------------------------
  function json_to_map(string: Nullable<string>) {
  ...
  function retrieve_SVs() {
    ...
    return json_to_map(storage_string)
  }
  // #endregion -------------------------------------------------------------------------------------
  // #region SV - update ----------------------------------------------------------------------------
  function update_SVs(provided_values: Map<string, string>, opts?: { previous_values?: Map<string, string> }) {
    const current_values = opts?.previous_values ?? retrieve_SVs()
  ...
  // #endregion -------------------------------------------------------------------------------------
  // #region TA - update ----------------------------------------------------------------------------
  function update_TAs(provided_values: Map<string, string>, opts?: { previous_values?: Map<string, string> }) {
    const current_values = opts?.previous_values ?? retrieve_TAs()
  ...
  // #endregion -------------------------------------------------------------------------------------
  // #region SM - update ----------------------------------------------------------------------------
  function update_SM(provided_value: Nullable<string>, opts?: { previous_value?: Nullable<string> }): SM_Update {
    const curr = sanitize_SM(opts?.previous_value ?? retrieve_SM())
  ...
  // #endregion -------------------------------------------------------------------------------------
  // #region CS - update ----------------------------------------------------------------------------
  function update_CS(prov_value: Nullable<string>, opts?: { previous_value?: Nullable<string> }): CS_Update {
    const { mode, CS: curr_CS, is_mode_handled } = sanitize_CS({ mode: prov_value, value: opts?.previous_value ?? retrieve_CS() })
  ...
  function handle_SM_change({ newValue, oldValue }: Omit<Custom_SE['detail'], 'key'>) {
    apply_CM(newValue, { previous_value: oldValue })
    apply_HVs(json_to_map(JSON.stringify({ mode: newValue })), { previous_values: json_to_map(JSON.stringify({ mode: oldValue })) })
  }
  // #endregion -------------------------------------------------------------------------------------
  // #region SVs - change handler -------------------------------------------------------------------
  function handle_SVs_change({ newValue, oldValue }: Omit<Custom_SE['detail'], 'key'>) {
    const new_values = json_to_map(newValue)
    const old_values = json_to_map(oldValue)
    apply_HVs(new_values, { previous_values: old_values })
    apply_CM(new_values.get('mode'), { previous_value: old_values.get('mode') })
  }
  // #endregion -------------------------------------------------------------------------------------
  // #region NATIVE_SE - listener -------------------------------------------------------------------
  function native_SE_listener({ key, newValue, oldValue }: StorageEvent) {
    if (key === mode_SK) handle_SM_change({ newValue, oldValue })
    else if (key === config_SK) handle_SVs_change({ newValue, oldValue })
  }
  // #endregion -------------------------------------------------------------------------------------
  ...
  // #endregion -------------------------------------------------------------------------------------
  // #region CM - applier ---------------------------------------------------------------------------
  function apply_CM(value: Nullable<string>, opts?: { previous_value?: Nullable<string> }) {
    update_SM(value, opts)
    update_CS(value, opts)
    toggle_MC(value)
  }
  // #endregion -------------------------------------------------------------------------------------
  // #region HVs - applier --------------------------------------------------------------------------
  function apply_HVs(values: Map<string, string>, opts?: { previous_values?: Map<string, string> }) {
    console.log(update_SVs(values, opts))
    update_TAs(values, opts)
  }
  // #endregion -------------------------------------------------------------------------------------
  // #region INITIALIZATION -------------------------------------------------------------------------
  function init() {
    const retrieved_SVs = retrieve_SVs()
    apply_HVs(retrieved_SVs)
    apply_CM(retrieved_SVs.get('mode'))
